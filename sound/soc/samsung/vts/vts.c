...
#define PAD_RETENTION_VTS_OPTION  (0x28A0)
...
#define PAD_RETENTION_VTS_OPTION  (0x2c20) // VTS_OUT : PAD__RTO
	...
	if (sync) {
		...
		for (i = 1000; i && (*state != SEND_MSG_OK) &&
			...
			vts_dev_dbg(dev, "%s ACK-value: 0x%08x\n", VTS_TAG_IPC,
				ack_value);
			udelay(90);		/* 50 -> 90 for 3rd algorithm */
		}
		if (!i) {
...
{
#if IS_ENABLED(CONFIG_SOC_S5E9815)	/* Only ERD 0.0 */
	void __iomem *sysreg_con;
	void __iomem *sysreg_dat;
	void __iomem *sysreg_conpdn;
	u32 val;
#endif
...
#if IS_ENABLED(CONFIG_SOC_S5E9815)	/* Only ERD 0.0 */
		/* Setting RESETB gpio : GPIO low -> high */
		sysreg_con = ioremap(0x11580000 + 0x0000, SZ_32);
		val = readl(sysreg_con);
		val = (val & ~0x0F000000) | 0x01000000;
		writel(val, sysreg_con);

		sysreg_dat = ioremap(0x11580000 + 0x0004, SZ_32);
		val = readl(sysreg_dat);
		val = val | 0x00000040;
		writel(val, sysreg_dat);
#endif
		exynos_pmu_update(PAD_RETENTION_VTS_OPTION,
			0x1 << 11, 0x1 << 11);
#if IS_ENABLED(CONFIG_SOC_S5E9815)	/* Only ERD 0.0 */
		/* GPIO conpdn : output high */
		sysreg_conpdn = ioremap(0x11580000 + 0x0010, SZ_32);
		val = readl(sysreg_conpdn);
		val = (val & ~0x00003000) | 0x00001000;
		writel(val, sysreg_conpdn);
#endif
#endif
	}
}
...
#if IS_ENABLED(CONFIG_SOC_S5E9815)
	writel(0x10A00, sfr_base + 0x8);
#else
...
bool vts_runtime_get_sync(void)
{
	struct vts_data *data = p_vts_data;
	struct platform_device *pdev = data->pdev;
	struct device *dev = &pdev->dev;

	vts_dev_info(dev, "%s : %d\n", __func__, pm_runtime_get_sync(dev));
	return 1;
}
EXPORT_SYMBOL(vts_runtime_get_sync);

bool vts_runtime_put_sync(void)
{
	struct vts_data *data = p_vts_data;
	struct platform_device *pdev = data->pdev;
	struct device *dev = &pdev->dev;
	...
	if (pm_runtime_active(dev))
		vts_dev_info(dev, "%s - %d\n", __func__, pm_runtime_put(dev));
	return 1;
}
EXPORT_SYMBOL(vts_runtime_put_sync);
...
static const char * const vtsadjust_freq_text[] = {
	"F60M", "F120M", "F180M", "F360M"
};
...
static SOC_ENUM_SINGLE_EXT_DECL(vtsadjust_freq_enum, vtsadjust_freq_text);
...
static int vts_start_recognization(struct device *dev, int start)
{
	...
	if (start) {
		...
		vts_dev_info(dev, "%s exec_mode %d active_trig :%d\n",
			     __func__, data->exec_mode, active_trigger);
		...
		if (!(data->exec_mode & (0x1 << VTS_SOUND_DETECT_MODE))) {
			if (active_trigger == TRIGGER_SVOICE &&
				...
				vts_dev_info(dev, "svoice.bin Binary uploaded size=%zu\n",
					data->svoice_info.actual_sz);
				vts_dev_info(dev, "3RD Party Detect Mode\n");
		...
		if (vts_pcm_dump_get_file_started(0)) {
			...
			values[1] = 0;
			values[2] = 0;
			vts_dev_info(dev, "VTS Recoding DUMP Start\n");
		...
		if (vts_pcm_dump_get_file_started(0)) {
			...
			values[1] = 0;
			values[2] = 0;
			vts_dev_info(dev, "VTS Recoding DUMP Stop\n");
			vts_start_ipc_transaction(dev, data,
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (data->voicecall_enabled) {
		u32 keyword_type = 1;
		char env[100] = {0,}, *env2 = "SOURCE=VTS";
		char *envp[3] = {env, env2, NULL};
		struct device *dev = &data->pdev->dev;
...
static int get_keyword_length(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct vts_data *data = p_vts_data;

	ucontrol->value.integer.value[0] = data->kw_length;
	vts_dev_info(component->dev, "get Keyword Length: %d\n", data->kw_length);
	return 0;
}

static int set_keyword_length(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct vts_data *data = p_vts_data;

	data->kw_length = ucontrol->value.integer.value[0];

	vts_dev_info(component->dev, "set Keyword Length to: %d\n", data->kw_length);
...
static int get_vtsadjust_freq(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component
		= snd_soc_kcontrol_component(kcontrol);
...
/* convert clock rate to kcontrol value "60M", "120M", "180M", "360M" */
	val = data->sysclk_rate / 60000000;
	...
	vts_dev_info(component->dev, "GET VTS Basic Frequence: %ld\n",
			data->sysclk_rate);

	return 0;
}

static int set_vtsadjust_freq(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component
		= snd_soc_kcontrol_component(kcontrol);
	struct device *dev = component->dev;
	struct vts_data *data = p_vts_data;
	unsigned long freq = 120000000;
	int val = ucontrol->value.integer.value[0];

/* convert kcontrol value to clock rate "60M", "120M", "180M", "360M" */
	if (val >= 0 && val < 3)
		val = val + 1;
	else if (val == 3)
		val = val + 3;
	...
	data->sysclk_rate = freq;

	vts_dev_info(dev, "SET VTS Basic Frequence: %ld\n",
			data->sysclk_rate);
...
static const struct snd_kcontrol_new vts_controls[] = {
	...
	SOC_SINGLE_EXT("Keyword Length",
		SND_SOC_NOPM,
		0, 140800, 0,
		get_keyword_length, set_keyword_length),
	SOC_ENUM_EXT("Force Reset", vtsforce_reset_enum,
		get_vtsforce_reset, set_vtsforce_reset),
	/* vivo add for adjust frequency */
	SOC_ENUM_EXT("Basic Frequency", vtsadjust_freq_enum,
		get_vtsadjust_freq, set_vtsadjust_freq),
};
	...
	struct platform_device *pdev, int micconf_type, bool enable)
{
	...
	if (enable) {
		if (!data->micclk_init_cnt) {
			...
			/* Simultaneous use with Serial LIF */
...
#ifdef DEF_HARDFAULT_RECOVERY
	/* For hardfault, Notifiy ST HAL to recovery */
	if (error_code >= 0 && error_code <= 4) {
		vts_dev_info(dev, "%s: try to recover vts\n", __func__);
...
static irqreturn_t vts_voice_triggered_handler(int irq, void *dev_id)
{
	...
	if (data->mic_ready & (0x1 << VTS_MICCONF_FOR_TRIGGER) ||
		...
		if (!(data->exec_mode & (0x1 << VTS_SOUND_DETECT_MODE))) {
			data->triggered_keyword_type = id[0];
		} else if (data->exec_mode & (0x1 << VTS_SOUND_DETECT_MODE)) {
			data->triggered_keyword_type = 4; // LPSD (temporary value)
		} else {
			vts_dev_warn(dev, "Unknown VTS Execution Mode!!\n");
		}

		data->poll_event_type |= EVENT_TRIGGERED|EVENT_READY;
		wake_up(&data->poll_wait_queue);
		__pm_wakeup_event(data->wake_lock, VTS_TRIGGERED_TIMEOUT_MS);
...
static irqreturn_t vts_trigger_period_elapsed_handler(int irq, void *dev_id)
{
	...
	if (data->mic_ready & (0x1 << VTS_MICCONF_FOR_TRIGGER) ||
		...
		if (vts_pcm_dump_get_file_started(1))
			vts_pcm_dump_period_elapsed(1, platform_data->pointer);
#endif
...
#ifdef DEF_VTS_PCM_DUMP
		if (vts_pcm_dump_get_file_started(0))
			vts_pcm_dump_period_elapsed(0, platform_data->pointer);
#endif
		snd_pcm_period_elapsed(platform_data->substream);
	}
...
//	u32 values[3] = {0, 0, 0};
	...
	if (data->vts_ready) {
		if (data->running &&
			data->vts_state == VTS_STATE_RECOG_TRIGGERED) {
			vts_dev_err(dev, "%s VTS_STATE_RECOG_TRIGGERED\n", __func__);
		}
...
mailbox_print_sfr(pdev);
	...
	result = clk_set_rate(data->clk_sys, data->sysclk_origin_rate);
	if (result < 0) {
		vts_dev_err(dev, "clk_set_rate: %d\n", result);
		return -1;
	}
	vts_dev_info(dev, "Revert System Clock : %ld\n", data->sysclk_origin_rate);
...
#endif
	vts_dev_info(dev, "fw log file : %d", data->fw_logfile_enabled);
	vts_dev_info(dev, "fw logger : %d", data->fw_logger_enabled);
...
int vts_start_runtime_resume(struct device *dev, int skip_log)
{
	...
	vts_dev_info(dev, "%s\n", __func__);
	vts_dev_info(dev, "Origin System Clock : %ld\n", data->sysclk_origin_rate);
...
#if !(IS_ENABLED(CONFIG_SOC_S5E9815))	// to check
	/* Send Get Google Version */
	...
	if (data->google_version == 0) {
		...
		if (result < 0)
			vts_dev_err(dev, "VTS_IRQ_AP_GET_VERSION ipc failed\n");
	}
#endif
	ver = (char *)(&data->vtsfw_version);
	vts_dev_info(dev, "Firmware version: (%c%c%c%c) / library version: (0x%x)\n",
		ver[3], ver[2], ver[1], ver[0], data->vtsdetectlib_version);

	/* Configure select sys clock rate */
	vts_clk_set_rate(dev, data->syssel_rate);

	/* for changing system clk */
	old_rate = clk_get_rate(data->clk_sys);
	result = clk_set_rate(data->clk_sys, data->sysclk_rate);
	if (result < 0) {
		vts_dev_err(dev, "clk_set_rate: %d\n", result);
		goto error_clk;
	}
	data->sysclk_rate = clk_get_rate(data->clk_sys);
	vts_dev_info(dev, "Change System Clock : %ld -> %ld\n",
		old_rate, data->sysclk_rate);

	vts_dev_info(dev, "%s : vts_set_baaw\n", __func__);
		...
		unsigned int cmd, int __user *_arg)
{
	...
	case VTSDRV_MISC_IOCTL_WRITE_EXIT_POLLING:
		...
		wake_up(&data->poll_wait_queue);
		break;
	case VTSDRV_MISC_IOCTL_READ_TRIGGERED_KEYWORD:
		put_user(data->triggered_keyword_type, (int __user *)_arg);
		data->triggered_keyword_type = TRIGGER_NONE;
		break;
	default:
...
static int vts_itmon_notifier(struct notifier_block *nb, unsigned long action, void *nb_data)
{
	...
	if (itmon_data && itmon_data->dest
	    && (strncmp("VTS", itmon_data->dest, sizeof("VTS") - 1) == 0)) {
		vts_dev_info(dev, "%s(%lu)\n", __func__, action);
		/* Dump VTS GPR register & SRAM */
...
#if IS_ENABLED(SND_SOC_SAMSUNG_VTS_S_LIF)
...
static int samsung_vts_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(data->sicd_base)) {
		vts_dev_warn(dev, "sicd is not support\n",
			"vtsfw_version");
		data->sicd_base = NULL;
	}
	...
	data->sysclk_origin_rate = clk_get_rate(data->clk_sys);
	data->sysclk_rate = data->sysclk_origin_rate;
...
//#if IS_ENABLED(CONFIG_SOC_S5E9815)
//MODULE_SOFTDEP("post: snd-soc-s5m3700");
//#endif
