...
#define ABOX_PAD_NORMAL				(0x1d20)
#define ABOX_PAD_NORMAL_MASK			(0x00000800)
#define ABOX_OPTION				(0x340c)
...
#define IPC_RETRY			(10)
#define SZ_96				(96)
#define RUNTIME_SUSPEND_RETRY		(5)
...
#ifdef CONFIG_SND_SOC_SAMSUNG_ABOX_FREE_RMEM
unsigned int debug_level;

unsigned int abox_get_debug_level(void)
{
	return debug_level;
}

static int __init get_debug_level(char *str)
{
...
__setup("androidboot.debug_level=", get_debug_level);
#endif
...
//			(target_pll / 10000000)) {
	...
	if (data->uaif_max_div <= 32) {
		...
		abox_info(dev, "abox rate(%d), width(%d), channels(%d)\n",
				rate, width, channels);
	...
	abox_info(dev, "abox set audif_rate(%d)\n", audif_rate);
...
static const struct reg_default abox_timer_reg_defaults_5515[] = {
	{0x0600, 0x00000000},
	{0x0604, 0x00000000},
	{0x0608, 0x00000000},
	{0x060C, 0x00000000},
...
static struct regmap_config abox_timer_regmap_config = {
	...
	.reg_defaults = abox_timer_reg_defaults_5515,
	.num_reg_defaults = ARRAY_SIZE(abox_timer_reg_defaults_5515),
		...
		bool on)
{
	...
	do {
		...
		val = readl_phys(0x13a50014);
		abox_info(dev, "abox 0x13a50014(0x%lu), ret(%d)\n", val, ret);
		val &= ABOX_SYSPOWER_STATUS_MASK;
		if (local_clock() > timeout) {
			abox_info(dev, "abox lck(%u), timeout(%u), val(0x%lu)\n",
					local_clock(), timeout, val);
...
void abox_request_dram_on(struct device *dev, unsigned int id, bool on)
{
	...
	abox_info(dev, "%s val(%lx)\n", __func__, val);
	writel_phys(val, 0x13a50010);
	read_val = readl_phys(0x13a50010);
	if (val != read_val) {
		abox_err(dev, "syspower write failed: %lx\n", read_val);
		return;
	}
...
int abox_iommu_unmap(struct device *dev, unsigned long iova)
{
	...
	if (!data->enabled) {
		abox_info(dev, "abox not unmap(%#lx) : abox not enabled\n", iova);
		return ret;
	}
	...
	list_for_each_entry(mapping, &data->iommu_maps, list) {
		...
		exynos_sysmmu_tlb_invalidate(data->iommu_domain, iova, mapping->bytes);
		devm_kfree(dev, mapping);
		break;
	}
	spin_unlock_irqrestore(&data->iommu_lock, flags);
...
static int abox_cpu_suspend_complete(struct device *dev)
{
	...
	regmap_read(regmap, ABOX_TIMER_PRESET_LSB(1), &value);
	abox_info(dev, "%s be(%d)\n", __func__, value);
		...
		struct abox_extra_firmware *efw)
{
	...
	abox_info(dev, "%s is downloaded %u, %#x\n", efw->name,
			efw->area, efw->offset);
unlock:
	mutex_unlock(&efw->lock);
}
...
static int abox_enable(struct device *dev)
{
...
#ifdef CONFIG_SND_SOC_ABOX_CP
			memcpy_toio(data->drv_args_addr, "vss=true",
					data->drv_args_size);
#else
			memcpy_toio(data->drv_args_addr, "vss=false",
					data->drv_args_size);
#endif
		}
		if (data->drv_args_addr_slog && data->drv_args_size_slog) {
			memcpy_toio(data->drv_args_addr_slog,
					data->modify_slog_size,
 					data->drv_args_size_slog);
	...
	abox_info(dev, "0x12863400(0x%lu)\n", readl_phys(0x12863400));
	if (has_reset) {
		abox_cpu_power(true);
		writel_phys(0x1, 0x12863400);
		abox_cpu_enable(true);
	}
	abox_info(dev, "0x12863400(0x%lu)\n", readl_phys(0x12863400));
		...
		unsigned long action, void *nb_data)
{
	...
	abox_info(dev, "pm qos aud: %ldkHz\n", value);
...
static int abox_dma_progress_check(struct abox_data *data)
{
	unsigned int val = 0;
	int i, ret;
	...
	for (i = 0; i < RDMA_COUNT; i++) {
		...
		ret = !!(val & ABOX_RDMA_PROGRESS_MASK);
		if (ret) {
			abox_info(data->dev, "rdma(%d) progress: %d\n", i, ret);
			return ret;
		}
	}
	...
	for (i = 0; i < WDMA_COUNT; i++) {
		...
		ret = !!(val & ABOX_WDMA_PROGRESS_MASK);
		if (ret) {
			abox_info(data->dev, "wdma(%d) progress: %d\n", i, val);
			return ret;
		}
	}
	return 0;
	...
	case PM_SUSPEND_PREPARE:
		...
		if (abox_is_clearable(dev, data)) {
			...
			if (ret < 0) {
				...
				if (retry_cnt > RUNTIME_SUSPEND_RETRY &&
						!abox_dma_progress_check(data)) {
					retry_cnt = 0;
					msg.ipcid = IPC_SYSTEM;
					system->msgtype = ABOX_SUSPEND_FAIL_FORCE_CLEAR;
					abox_request_ipc(dev, msg.ipcid,
							&msg, sizeof(msg), 1, 1);
				}
				retry_cnt++;
				return NOTIFY_BAD;
...
static int abox_sysmmu_fault_handler(struct iommu_domain *domain,
	struct device *dev, unsigned long iova, int flags, void *token)
{
	abox_info(dev, "%s : watchdog reset\n", __func__);

	abox_dbg_print_gpr(dev, p_abox_data);
	abox_dbg_dump_gpr(dev, p_abox_data, ABOX_DBG_DUMP_KERNEL, "sysmmu");
	abox_dbg_dump_mem(dev, p_abox_data, ABOX_DBG_DUMP_KERNEL, "sysmmu");
...
#ifdef CONFIG_SND_SOC_SAMSUNG_AUDIO
	if (system_msg->msgtype == ABOX_RESET_VSS ||
		system_msg->msgtype == ABOX_STOP_VSS ||
		system_msg->msgtype == ABOX_WATCHDOG_VSS)
...
#else
#define abox_memlog_register(data) do { } while(0)
#endif
...
static int samsung_abox_probe(struct platform_device *pdev)
{
	...
	iovmm_set_fault_handler(dev, abox_sysmmu_fault_handler, NULL);
/*
	if (!abox_dram_rmem) {
		np_tmp = of_parse_phandle(np, "memory-region", 0);
		if (!np_tmp)
			return -ENODEV;
		abox_dram_rmem = of_reserved_mem_lookup(np_tmp);
	}
*/
	if (!abox_dram_rmem) {
	...
	data->dram_phys = abox_dram_rmem->base;
	...
	if (paddr) {
		...
		abox_info(dev, "not vss firmware virtual\n");
	}
#endif
	...
	ret = of_samsung_property_read_u32(dev, np, "uaif_max_div",
			&data->uaif_max_div);
	if (ret < 0)
	...
	abox_of_get_addr(data, np, "samsung,drv-bootargs-area",
			&data->drv_args_addr, NULL, &data->drv_args_size);
	abox_of_get_addr(data, np, "samsung,drv-bootargs-slog",
			&data->drv_args_addr_slog, NULL,
			&data->drv_args_size_slog);
	ret = of_property_read_string(np, "samsung,modify-slog-size",
			&data->modify_slog_size);
#ifdef CONFIG_SND_SOC_SAMSUNG_ABOX_FREE_RMEM
	if (ret < 0 || abox_get_debug_level() != 0)
