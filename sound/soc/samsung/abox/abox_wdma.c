...
static const struct snd_kcontrol_new abox_wdma_bit_controls_3_1_2[] = {
...
static int abox_wdma_probe(struct snd_soc_component *cmpnt)
{
	...
	if (ABOX_SOC_VERSION(3, 1, 2) == CONFIG_SND_SOC_SAMSUNG_ABOX_VERSION)
		snd_soc_add_component_controls(cmpnt,
				abox_wdma_bit_controls_3_1_2,
				ARRAY_SIZE(abox_wdma_bit_controls_3_1_2));
	else
		snd_soc_add_component_controls(cmpnt,
		...
		unsigned int reg, unsigned int val)
{
	...
	if (reg == DMA_REG_CTRL) {
		if (pm_runtime_get_if_in_use(cmpnt->dev) > 0) {
			regcache_cache_bypass(abox_data->regmap, true);
			ret = regmap_update_bits(abox_data->regmap, base + reg,
					REG_CTRL_KERNEL_MASK, val);
			regcache_cache_bypass(abox_data->regmap, false);
...
static int abox_wdma_sbank_size_get(struct snd_kcontrol *kcontrol,
...
static int abox_wdma_sbank_size_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct device *dev = cmpnt->dev;
...
static const struct snd_kcontrol_new abox_wdma_controls[] = {
	...
	SOC_SINGLE_EXT("Sbank Size", 0, 0, SZ_512, 0, abox_wdma_sbank_size_get,
			abox_wdma_sbank_size_put),
};
