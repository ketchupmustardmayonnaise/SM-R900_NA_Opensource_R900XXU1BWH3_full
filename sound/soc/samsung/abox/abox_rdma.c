...
static int abox_rdma_progress(struct abox_dma_data *data)
{
	...
	abox_info(data->dev, "abox ABOX_RDMA_STATUS(0x%lu), id(%d)\n",
			readl_phys(0x13a50000 + ABOX_RDMA_STATUS(data->id)), data->id);
		...
		struct abox_dma_data *data)
{
	...
	while (abox_rdma_progress(data) ||
		...
		abox_info(dev, "abox r_disable lck(%u), timeout(%u)\n",
				local_clock(), timeout);
...
static const struct snd_kcontrol_new abox_rdma_bit_controls_3_1_2[] = {
...
static int abox_rdma_probe(struct snd_soc_component *cmpnt)
{
	...
	if (ABOX_SOC_VERSION(3, 1, 2) == CONFIG_SND_SOC_SAMSUNG_ABOX_VERSION)
		snd_soc_add_component_controls(cmpnt,
				abox_rdma_bit_controls_3_1_2,
				ARRAY_SIZE(abox_rdma_bit_controls_3_1_2));
	else
		snd_soc_add_component_controls(cmpnt,
