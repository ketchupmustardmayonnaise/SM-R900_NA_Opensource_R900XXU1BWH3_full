...
#ifdef CONFIG_SENSORS_SSP
	if (atomic_read(&chub->ssp_chub_sleep)) {
		nanohub_dev_info(chub->dev, "%s ssp_wakeup_by_me: status:0x%x\n",
				 __func__, status);
		chub->ssp_wakeup_by_chub_cnt++;
		atomic_set(&chub->ssp_chub_sleep, 0);
...
#ifdef CONFIG_SENSORS_SSP
	atomic_set(&ipc->ssp_chub_sleep, 1);
#endif
#ifdef CONFIG_SEC_PM_DEBUG
