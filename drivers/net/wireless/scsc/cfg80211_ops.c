		  ...
		  struct cfg80211_ap_settings *settings)
{
...
#ifdef CONFIG_SCSC_WLAN_SUPPORT_6G
	if (slsi_dev_6ghz_skip_acs() && settings->chandef.chan->band == NL80211_BAND_6GHZ) {
		if (sdev->supported_6g_160mhz)
			settings->chandef.width = NL80211_CHAN_WIDTH_160;
		else
			settings->chandef.width = NL80211_CHAN_WIDTH_80;
		SLSI_NET_DBG1(dev, SLSI_CFG80211, "update Bandwidth to %d for test\n", settings->chandef.width);
	}
#endif
	...
	if (settings->chandef.chan->band == NL80211_BAND_5GHZ) {
		r = slsi_ap_start_get_indoor_chan_5g(sdev, dev, settings, skip_indoor_check_for_wifi_sharing,
						     &indoor_channel);
		if (r != 0)
			goto exit_with_vif_mutex;
	}
