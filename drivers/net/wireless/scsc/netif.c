...
#elif defined(CONFIG_SOC_S5E9815) || defined(CONFIG_SOC_S5E8825) || defined(CONFIG_SOC_S5E9925)
...
#if defined(CONFIG_SOC_S5E9815) || defined(CONFIG_SOC_S5E8825) || defined(CONFIG_SOC_S5E9925)
...
#if CONFIG_SCSC_WLAN_MAX_INTERFACES >= 5 && defined(CONFIG_SCSC_WIFI_NAN_ENABLE)
...
#elif defined(CONFIG_SOC_EXYNOS9630) || defined(CONFIG_SOC_S5E8825) || defined(CONFIG_SOC_S5E9925)
#define SCSC_NETIF_NAPI_CPU_BIG                   7
	...
	if (!(SLSI_IS_VIF_INDEX_P2P(ndev_vif) || SLSI_IS_VIF_INDEX_NAN(ndev_vif))) {
		...
		if (napi_cpu_big_tput_in_mbps || rps_enable_tput_in_mbps) {
			...
			if (rps_enable_tput_in_mbps < napi_cpu_big_tput_in_mbps) {
				...
				mid_tput = (napi_cpu_big_tput_in_mbps * 1000 * 1000);
				high_tput = (rps_enable_tput_in_mbps * 1000 * 1000);
			}
...
int slsi_netif_init(struct slsi_dev *sdev)
{
	...
	if (slsi_netif_add_locked(sdev, "p2p%d", SLSI_NET_INDEX_P2P) != 0) {
		rtnl_lock();
		slsi_netif_remove_locked(sdev, sdev->netdev[SLSI_NET_INDEX_WLAN]);
		slsi_netif_remove_locked(sdev, sdev->netdev[SLSI_NET_INDEX_AP]);
		rtnl_unlock();
		SLSI_MUTEX_UNLOCK(sdev->netdev_add_remove_mutex);
		return -EINVAL;
	}
...
#endif
#endif
#if CONFIG_SCSC_WLAN_MAX_INTERFACES >= 5
