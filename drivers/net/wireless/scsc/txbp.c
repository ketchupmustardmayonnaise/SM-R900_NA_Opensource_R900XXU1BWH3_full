...
static int bp_log = LOG_FILTER_Q_STATUS | LOG_FILTER_TX_FAILURE | LOG_FILTER_SHOW_COD;
...
static __always_inline void slsi_txbp_check_q_status(struct net_device *dev, struct tx_netdev_data *tx_priv, int qidx)
{
	...
	if (__netif_subqueue_stopped(dev, qidx)) {
		...
		if (READ_ONCE(tx_priv->q[qidx].qlen) < max_q_len || !READ_ONCE(tx_priv->q[qidx].qlen)) {
...
__always_inline bool slsi_vif_activated_post(struct slsi_dev *sdev, struct net_device *dev, struct netdev_vif *ndev_vif)
{
	...
	tx_info->target_cpu = 5;
	bp_fixed_cpu = tx_info->target_cpu;
#endif
	tx_info->sdev = sdev;
	tx_info->ndev = dev;
	...
	SLSI_NET_INFO(dev, "BP: active interfaces %u\n", txbp_priv.vif_cnt);
					       ...
					       struct netdev_vif *ndev_vif)
{
	...
	kfree(tx_priv);
	SLSI_NET_INFO(dev, "BP: deactive interfaces %u\n", txbp_priv.vif_cnt);
}
...
static __always_inline void slsi_txbp_schedule_napi(struct net_device *dev, struct tx_netdev_data *tx_priv)
{
	...
	if (bp_log & LOG_FILTER_TX_NAPI)
		SLSI_NET_INFO(dev, "BP: schedule tx\n");
