...
static inline bool is_valid_board_for_heart_small(struct panel_device *panel)
{
	// boe panel only wonking in 2nd pmic(richtek), board rev 14 ~ 255
	if ((panel->panel_data.id[1] & 0x60) == 0x00) {			// heart small
		if (panel->panel_data.board_revision >= 14) { 		// 2nd pmic
			return true;
		}
	}
	panel_info("board(%d) and panel(%x %x %x) mis-match\n", panel->panel_data.board_revision,
			panel->panel_data.id[0], panel->panel_data.id[1], panel->panel_data.id[2]);
	return false;
}
#endif
