...
#if defined(CONFIG_SOC_S5E5515) && defined(CONFIG_ARM_EXYNOS_DEVFREQ)
...
static int decon_blank(int blank_mode, struct fb_info *info)
{
	...
	decon_info("decon-%d %s mode: %d type (0: DSI, 1: eDP, 2:DP, 3: WB) : %s\n",
			decon->id,
...
static int decon_fb_alloc_memory(struct decon_device *decon, struct decon_win *win)
{
	...
	dev_info(decon->dev, "fb start addr = 0x%x size:0x%x\n", (u32)fbi->fix.smem_start, fbi->fix.smem_len);
