...
enum ecg_cmd_data {
	...
	ECG_CMD_NOISE_EXT,
	ECG_CMD_AC_TEST_5HZ,
	ECG_CMD_ADC_OFFSET_NODE,
	ECG_CMD_OTP = 9,
	ECG_CMD_POSITIVE_THRESHOLD_NODE,
	ECG_CMD_NEGATIVE_THRESHOLD_NODE = 11,
};
...
static ssize_t ecg_protocol_version_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ssp_data *data = dev_get_drvdata(dev);
	int ret = 0;
	...
	ret = ssp_send_read_factory_cmd(data, HRM_ECG_LIB_SENSOR, (ECG_CMD_PROTOCOL<< 8) | ECG_READ_CMD, (char *)&version, sizeof(version), 5000, 4, __func__);
	if(ret != SUCCESS) {
		version = 255;
	}
	pr_info("[SSP] %s value: %d %d", __func__, version, version & 0xFF);
	return snprintf(buf, PAGE_SIZE, "%d\n", version & 0xFF);
}

static ssize_t ecg_protocol_version_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	uint8_t body = 0;
	uint8_t tmp[25] = {0,};
	...
	tkn = strsep(&strptr, " ");
	if (!tkn) {
		pr_err("[SSP] %s - body NULL!\n", __func__);
		return -EINVAL;
	}
	if (kstrtou8(tkn, 0, &body)) {
		pr_err("[SSP] %s - body INVAL!\n", __func__);
		return -EINVAL;
	}

	pr_info("[SSP] %s - body: %d", __func__, body);

	ssp_send_write_factory_cmd(data, HRM_ECG_LIB_SENSOR, (ECG_CMD_PROTOCOL << 8) | ECG_WRITE_CMD, (char *)&body, sizeof(body), __func__);

	return size;
}

static ssize_t ecg_adc_offset_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ssp_data *data = dev_get_drvdata(dev);
	int ret = 0;
	...
	ret = ssp_send_read_factory_cmd(data, HRM_ECG_LIB_SENSOR, (ECG_CMD_ADC_OFFSET_NODE<< 8) | ECG_READ_CMD, (char *)&adc_offset, sizeof(adc_offset), 5000, 4, __func__);
	if(ret != SUCCESS) {
		adc_offset = 0;
	}
...
static ssize_t ecg_positive_threshold_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ecg_threshold threshold;
	struct ssp_data *data = dev_get_drvdata(dev);
	int ret = 0;

	ret = ssp_send_read_factory_cmd(data, HRM_ECG_LIB_SENSOR, (ECG_CMD_POSITIVE_THRESHOLD_NODE<< 8) | ECG_READ_CMD, (char *)&threshold, sizeof(threshold), 5000, 4, __func__);
	if(ret != SUCCESS) {
		threshold.value = 0;
	}
...
static ssize_t ecg_negative_threshold_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ecg_threshold threshold;
	struct ssp_data *data = dev_get_drvdata(dev);
	...
	ret = ssp_send_read_factory_cmd(data, HRM_ECG_LIB_SENSOR, (ECG_CMD_NEGATIVE_THRESHOLD_NODE<< 8) | ECG_READ_CMD, (char *)&threshold, sizeof(threshold), 5000, 4, __func__);
	if(ret != SUCCESS) {
		threshold.value = 0;
	}
...
static DEVICE_ATTR(protocol_version, S_IRUGO | S_IWUSR | S_IWGRP, ecg_protocol_version_show, ecg_protocol_version_store);
...
static DEVICE_ATTR(adc_offset, S_IRUGO, ecg_adc_offset_show, NULL);
static DEVICE_ATTR(otp, S_IRUGO, ecg_otp_show, NULL);
static DEVICE_ATTR(positive_threshold, S_IRUGO, ecg_positive_threshold_show, NULL);
static DEVICE_ATTR(negative_threshold, S_IRUGO, ecg_negative_threshold_show, NULL);
...
static struct device_attribute *ecg_attrs[] = {
	...
	&dev_attr_adc_offset,
	&dev_attr_otp,
	&dev_attr_positive_threshold,
	&dev_attr_negative_threshold,
	NULL,
};
