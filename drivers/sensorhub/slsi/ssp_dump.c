...
void write_ssp_dump_file(struct ssp_data *data, char *info, void *dump, int size, int type, u32 *gpr, int num_gpr, u32 *hardfault_info)
{
	...
	if (sensorhub_info != NULL) {
		...
		data->crash_mini_dump.month = t.tm_mon+1;
		data->crash_mini_dump.day = t.tm_mday;
		data->crash_mini_dump.hour = t.tm_hour;
		data->crash_mini_dump.min = t.tm_min;
		data->crash_mini_dump.sec = t.tm_sec;
...
void ssp_dump_file(struct ssp_data *data, int *size)
{
	...
	if (bSdumpRefreshed) {
		ssp_dump_file_store(data, get_sensorhub_dram_ptr(), get_sensorhub_dram_size());
		*size = get_sensorhub_dram_size();
...
void ssp_sram_dump_store(struct ssp_data *data, int mode)
{
    ...
    mm_segment_t old_fs;
    int ret;
    void *buf = get_sensorhub_dram_ptr();
    int size = get_sensorhub_dram_size();
    struct timespec ts;
    struct rtc_time t;
