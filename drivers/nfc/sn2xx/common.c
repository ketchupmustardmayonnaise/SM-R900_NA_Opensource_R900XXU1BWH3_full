		 ...
		 uint8_t interface)
{
...
#else
	if (of_property_read_bool(np, "pn547,ldo_control"))
		nfc_configs->nfc_pvdd = regulator_get(dev, "nfc_pvdd");
	else
		nfc_gpio->pvdd = of_get_named_gpio(np, DTS_PVDD_GPIO_STR, 0);
#endif
	...
	if (of_property_read_bool(np, "pn547,ldo_control")) {
		...
		if (!gpio_is_valid(nfc_gpio->pvdd))
...
void nfc_power_control(struct nfc_dev *nfc_dev)
{
	...
	} else if(gpio_is_valid(nfc_gpio->pvdd)) {
		ret = gpio_request(nfc_gpio->pvdd, "nfc_pvdd_gpio");
		if (ret) {
			NFC_LOG_ERR("%s failed to get gpio pvdd-gpio\n", __func__);
			gpio_free(nfc_gpio->pvdd);
		}
		gpio_direction_output(nfc_gpio->pvdd, 1);
		g_is_nfc_pvdd_enabled = 1;
		NFC_LOG_INFO("%s pvdd-gpio:%d\n", __func__, nfc_gpio->pvdd);
	}
...
void nfc_print_status(void)
{
	...
	if (!IS_ERR_OR_NULL(nfc_configs->nfc_pvdd))
		...
		pvdd = get_valid_gpio(nfc_gpio->pvdd);
	else
