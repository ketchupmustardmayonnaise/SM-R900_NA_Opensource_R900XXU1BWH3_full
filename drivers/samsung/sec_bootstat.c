...
void sec_bootstat_add_initcall(const char *s)
{
	...
	for (i = 0; i < ARRAY_SIZE(sec_boot_initcall); i++) {
		if (!strcmp(s, sec_boot_initcall[i].string)) {
			...
			sec_bootstat_get_cpuinfo(sec_boot_initcall[i].freq, &sec_boot_initcall[i].online);
			sec_bootstat_get_thermal(sec_boot_initcall[i].temp);
...
{
	seq_printf(m, "%-50s %6u %6u %6d %4d L%d%d%d%d M%d%d%d B%d %2d %2d %2d %2d %2d %2d\n",
...
static int sec_boot_stat_proc_show(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "boot event                                           time  ktime  delta f_c0 online_mask   B  M  L  G  I  N\n");
	seq_puts(m, "----------------------------------------------------------------------------------------------------------------------\n");
...
static int boot_time_bl1;
static int boot_time_bl2;
static int boot_time_bl3;

static int __init secdbg_set_boot_time_bl1(char *arg)
{
	get_option(&arg, &boot_time_bl1);
	return 0;
}

static int __init secdbg_set_boot_time_bl2(char *arg)
{
	get_option(&arg, &boot_time_bl2);
	return 0;
}

static int __init secdbg_set_boot_time_bl3(char *arg)
{
	get_option(&arg, &boot_time_bl3);
	return 0;
}

early_param("sec_bootstat.boot_time_bl1", secdbg_set_boot_time_bl1);
early_param("sec_bootstat.boot_time_bl2", secdbg_set_boot_time_bl2);
early_param("sec_bootstat.boot_time_bl3", secdbg_set_boot_time_bl3);
...
static int sec_enhanced_boot_stat_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%-90s %6s %6s %6s %6s\n", "Boot Events", "time", "ktime", "delta", "f_c0");
	seq_puts(m, "-----------------------------------------------------------------------------------------------------------------------\n");
	seq_puts(m, "BOOTLOADER - KERNEL\n");
	seq_puts(m, "-----------------------------------------------------------------------------------------------------------------------\n");
	seq_printf(m, "%-90s %6u %6u %6u %6d\n", "!@Boot_EBS_B: MCT_is_initialized", 0, 0, 0, 0);
	seq_printf(m, "%-90s %6u %6u %6u %6d\n", "!@Boot_EBS_B: boot_time_bl1", boot_time_bl1, 0, boot_time_bl1, 0);
	seq_printf(m, "%-90s %6u %6u %6u %6d\n", "!@Boot_EBS_B: boot_time_bl2", boot_time_bl2, 0, boot_time_bl2 - boot_time_bl1, 0);
	seq_printf(m, "%-90s %6u %6u %6u %6d\n", "!@Boot_EBS_B: boot_time_bl3", boot_time_bl3, 0, boot_time_bl3 - boot_time_bl2, 0);
	seq_printf(m, "%-90s %6u %6u %6u %6d\n", "!@Boot_EBS_B: start_kernel_timer", mct_start, 0, mct_start - boot_time_bl3, 0);

#ifdef CONFIG_SEC_BOOTSTAT
	for (i = 0; i < ARRAY_SIZE(sec_boot_initcall); i++) {
		seq_printf(m, "%-90s %6u %6u %6u %6d\n", sec_boot_initcall[i].string, sec_boot_initcall[i].time + mct_start, sec_boot_initcall[i].time,
				sec_boot_initcall[i].time - last_time, sec_boot_initcall[i].freq[0] / 1000);
		last_time = sec_boot_initcall[i].time;
	}
#endif
	seq_puts(m, "-----------------------------------------------------------------------------------------------------------------------\n");
	...
	list_for_each_entry_reverse (entry, &enhanced_boot_time_list, next){
		if (entry->buf[0] == '!') {
			...
			seq_printf(m, "%-90s %6u %6u %13d\n", entry->buf, entry->time + mct_start, entry->time, entry->freq[0] / 1000);
		}
	}
	return 0;
}
