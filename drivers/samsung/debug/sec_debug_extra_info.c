...
#define SEC_DEBUG_SHARED_MAGIC2 0x15001050
...
/* get reset_reason and pwr/pwroff src from bootloader by cmdline */
#define MAX_RR_PWRSRC	10
static char rr_pwrsrc[MAX_RR_PWRSRC + 1];
/*****************************************************************/
/*                        UNIT FUNCTIONS                         */
/*****************************************************************/
...
static void __init sec_debug_extra_info_buffer_init(void)
{
	...
	if (secdbg_base_get_buf_size(SDN_MAP_EXTRA_INFO) / 2 < item_size) {
		pr_err("%s: item size overflow: rsvd: %lu, item size: %lu\n",
			__func__, secdbg_base_get_buf_size(SDN_MAP_EXTRA_INFO) / 2, item_size);
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
		panic("%ssize error", __func__);
#endif
	}
...
static char *long_to_32digit(unsigned long long num, char *res, int buffer_size)
{
	int i = 0;
	long mod_num = num;

	while (mod_num && i < buffer_size - 1) {
		++i;
		mod_num /= 32;
	};
	res[i] = '\0';
...
#define EXIN_ID_LEN 10
static void __init sec_debug_set_extra_info_id(void)
{
	struct timespec ts;
	char digit32_id[EXIN_ID_LEN];

	getnstimeofday(&ts);
	long_to_32digit(ts.tv_nsec, digit32_id, EXIN_ID_LEN);

	set_bk_item_val("ID", SLOT_BK_32, "%s%s", digit32_id, EXTRA_VERSION);

	set_item_val("ASB", "%d", id_get_asb_ver());
	set_item_val("PSITE", "%d", id_get_product_line());
	set_item_val("DDRID", "%s", dram_info);
	set_item_val("DVS", "%d", asv_ids_information(dvs));
}
...
static void sec_debug_set_reset_reason_using_module_param(void)
{
	...
	long long_rr_pwrsrc;
	long reset_reason, offsrc, onsrc;

	kstrtol(rr_pwrsrc, 16, &long_rr_pwrsrc);

	reset_reason = (long_rr_pwrsrc & 0xff0000) >> 16;
	offsrc = (long_rr_pwrsrc & 0x00ff00) >> 8;
	onsrc = long_rr_pwrsrc & 0x0000ff;

	if (!get_bk_item("RR") && reset_reason > 0)
		set_bk_item_val("RR", SLOT_BK_32, "%cP", rr_c[reset_reason-1]);
	if (!get_bk_item("PWR"))
		set_bk_item_val("PWR", SLOT_BK_64, " %02X", (onsrc & 0xff));
	if (!get_bk_item("PWROFF"))
		set_bk_item_val("PWROFF", SLOT_BK_64, " %02X", (offsrc & 0xff));
}
...
static int _secdbg_exin_get_backtrace(char *fbuf, struct task_struct *task, struct stackframe *frame)
{
	char buf[64];
	int offset = 0;
	int sym_name_len;
...
void secdbg_exin_set_backtrace(struct pt_regs *regs)
{
	...
	_secdbg_exin_get_backtrace(fbuf, NULL, &frame);
...
void secdbg_exin_set_backtrace_task(struct task_struct *tsk)
{
	...
	_secdbg_exin_get_backtrace(fbuf, tsk, &frame);
...
void secdbg_exin_set_main_ocp(void)
{
...
static int __init secdbg_set_rr_pwrsrc(char *arg)
{
	if (strlen(arg) > MAX_RR_PWRSRC)
		return 0;

	memcpy(rr_pwrsrc, arg, (int)strlen(arg));
	return 0;
}

early_param("sec_debug.rr_pwrsrc", secdbg_set_rr_pwrsrc);
