...
extern struct exynos_pm_qos_request exynos5_g3d_mif_min_qos;
#endif
...
void gpu_dvfs_set_need_cpu_qos(struct kbase_device *kbdev, int need_cpu_qos)
{
	struct exynos_context *platform = (struct exynos_context *) kbdev->platform_context;
	char msg[32];
	char *env[2] = {msg, NULL};

	if((platform->min_lock == platform->gpu_neg_boost_clock && platform->max_lock == platform->gpu_neg_boost_clock)
		|| (platform->min_lock == platform->gpu_sustainable_clock && platform->max_lock == platform->gpu_sustainable_clock)){

		snprintf(msg, sizeof(msg), "GPU_REQ_NEGATIVE_BOOST=%d", need_cpu_qos);

		if (kobject_uevent_env(&kbdev->dev->kobj, KOBJ_CHANGE, env))
			dev_warn(kbdev->dev, "Failed to send uevent for negative boost");
	}
}
#endif
...
int gpu_vendor_dispatch(struct kbase_context *kctx, u32 flags)
{
	...
	case KBASE_FUNC_UNSET_VK_BOOST_LOCK:
		...
		case KBASE_FUNC_SET_NEGATIVE_BOOST_LOCK:
		{
			...
			gpu_dvfs_set_need_cpu_qos(kbdev, 1);
		...
		case KBASE_FUNC_UNSET_NEGATIVE_BOOST_LOCK:
		{
			...
			mutex_lock(&platform->gpu_negative_boost_lock);
			gpu_dvfs_set_need_cpu_qos(kbdev, 0);
...
void gpu_cl_boost_init(void *dev)
{
	...
	atomic_set(&kbdev->pm.backend.metrics.cnt_compute_jobs, 0);
	atomic_set(&kbdev->pm.backend.metrics.cnt_vertex_jobs, 0);
	atomic_set(&kbdev->pm.backend.metrics.cnt_fragment_jobs, 0);
	atomic_set(&kbdev->pm.backend.metrics.heavy_compute_util_count, 0);
}
...
void gpu_cl_boost_update_utilization(void *dev, void *atom, u64 microseconds_spent)
{
	...
	if (katom->core_req & BASE_JD_REQ_ONLY_COMPUTE) {
		...
		atomic_inc(&kbdev->pm.backend.metrics.cnt_compute_jobs);
		...
		atomic_inc(&kbdev->pm.backend.metrics.cnt_fragment_jobs);
		...
		atomic_inc(&kbdev->pm.backend.metrics.cnt_vertex_jobs);
	}
}
#endif
...
int gpu_pm_get_dvfs_utilisation(struct kbase_device *kbdev, int *util_gl_share, int util_cl_share[2])
{
	...
	int compute_cnt = 0, vertex_cnt = 0, fragment_cnt = 0, vf_ratio = 0;
	int heavy_compute_count = 0;
#endif

#ifdef CONFIG_MALI_LOG_FRAG_UTIL
	/* MALI_DVFS_EXPERIMENTAL */
	int i;
	int js_utilisation[BASE_JM_MAX_NR_SLOTS];
	struct exynos_context *platform = (struct exynos_context *)kbdev->platform_context;
#endif
	...
	if (kbdev->pm.backend.metrics.gpu_active) {
		...
		/* MALI_DVFS_EXPERIMENTAL */
		for (i = 0; i < 1; i++) {
			if (kbdev->pm.backend.metrics.active_js_ctx[i] == true)
				kbdev->pm.backend.metrics.values.busy_js[i] += ns_time;
			else
				kbdev->pm.backend.metrics.values.idle_js[i] += ns_time;
		}
#endif
		...
		/* MALI_DVFS_EXPERIMENTAL */
		for (i = 0; i < 1; i++) {
			kbdev->pm.backend.metrics.values.idle_js[i] += (u32) (ktime_to_ns(diff) >> KBASE_PM_TIME_SHIFT);
		}
#endif
		kbdev->pm.backend.metrics.time_period_start = now;
	}
	...
	/* MALI_DVFS_EXPERIMENTAL */
	for (i = 0; i < 1; i++) {
		js_utilisation[i] = (100 * kbdev->pm.backend.metrics.values.busy_js[i]) /
			(kbdev->pm.backend.metrics.values.idle_js[i] +
			 kbdev->pm.backend.metrics.values.busy_js[i]);
	}
	GPU_LOG(DVFS_INFO, DUMMY, 0u, 0u, "[FRAG] Freq= %d, Util= %d, JS0=%d\n", platform->cur_clock, utilisation, js_utilisation[0]);
//	printk("hjw_clock_%d_gpu_%d_js0_%d_js1_%d_js2_%d\n", platform->cur_clock, utilisation, js_utilisation[0], js_utilisation[1], js_utilisation[2]);
#endif

#if defined(CONFIG_MALI_NOTIFY_UTILISATION)
	notify_frag_utils_change(js_utilisation[0]); // Futher update utilizations
#endif
	...
	heavy_compute_count = atomic_read(&kbdev->pm.backend.metrics.heavy_compute_util_count);
	...
	if(total_time > 0 && fragment_time > 0) {
		compute_cnt = atomic_read(&kbdev->pm.backend.metrics.cnt_compute_jobs);
		vertex_cnt = atomic_read(&kbdev->pm.backend.metrics.cnt_vertex_jobs);
		fragment_cnt = atomic_read(&kbdev->pm.backend.metrics.cnt_fragment_jobs);
		...
		if(vf_ratio > 94 && ((vertex_time + compute_time)*100/fragment_time) > 60) {
			if(heavy_compute_count < 10) {
				heavy_compute_count++;
				atomic_inc(&kbdev->pm.backend.metrics.heavy_compute_util_count);
			}
		} else {
			if(heavy_compute_count > 0) {
				heavy_compute_count--;
				atomic_dec(&kbdev->pm.backend.metrics.heavy_compute_util_count);
			}
		}
	}

	update_gpu_data(js_utilisation[0], heavy_compute_count, platform->cur_clock);
#endif
out:
	...
	for (i = 0; i < BASE_JM_MAX_NR_SLOTS; i++) {
		kbdev->pm.backend.metrics.values.busy_js[i] = 0;
		kbdev->pm.backend.metrics.values.idle_js[i] = 0;
	}
#endif
