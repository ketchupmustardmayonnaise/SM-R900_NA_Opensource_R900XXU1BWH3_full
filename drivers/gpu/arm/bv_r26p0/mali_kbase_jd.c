...
#ifdef CONFIG_MALI_JD_RINGBUFFER
#define MAX_KATOM_ARRAY_SIZE	256

struct katom_ring {
	struct kbase_jd_atom *katom[MAX_KATOM_ARRAY_SIZE];
	u8 head;
	u8 tail;
	int is_working;
	/* spinlock_t eq_lock; */
	...
	struct timer_list remain_timer;
};
struct katom_ring kring;

struct kbase_jd_atom kring_temp_katom;
struct workqueue_struct *job_done_loop_wq;
KBASE_EXPORT_TEST_API(job_done_loop_wq);

void kbase_jd_done_worker_loop(struct work_struct *data);

int is_kring_empty(void)
{
	unsigned long flags;
	int ret = 0;

	spin_lock_irqsave(&kring.dq_lock, flags);
	if (kring.head == kring.tail)
		ret = 1;
	spin_unlock_irqrestore(&kring.dq_lock, flags);

	return ret;
}

static void kring_timer_func(struct timer_list *t)
{
	unsigned long flags;
	...
	dev_info(kring.kbdev->dev, "There are remain katoms.(queue_work)\n");
	spin_lock_irqsave(&kring.kbdev->hwaccess_lock, flags);
	queue_work(job_done_loop_wq, &kring_temp_katom.work);
	spin_unlock_irqrestore(&kring.kbdev->hwaccess_lock, flags);
}

static inline int kbase_kring_enqueue(struct kbase_jd_atom *katom)
{
	/*
	spin_lock(&kring.eq_lock);
	if (unlikely(((kring.tail + 1) % MAX_KATOM_ARRAY_SIZE) == kring.head)) {
		panic("KRing is FULL!!!\n");
	} else {
	*/
		kring.katom[kring.tail] = katom;
		/* Write-memory-barrier is required for CPU re-ordering but
		 * it will be replaced to retry from dequeue for performance.
		 * smp_wmb();
		 */
		kring.tail += 1;
		/*
		if (kring.tail == MAX_KATOM_ARRAY_SIZE)
			kring.tail = 0;
	}
	spin_unlock(&kring.eq_lock);
	*/

	return 0;
}

static inline struct kbase_jd_atom *kbase_kring_dequeue()
{
...
retry:
		/* printk("Kring DQ - %d\n", kring.head); */
		katom = kring.katom[kring.head];
		if (katom == NULL && counter < 20) {
			ndelay(10);
			counter++;
			goto retry;
		}
		kring.katom[kring.head] = NULL;
		kring.head++;
		/*
		if (kring.head == MAX_KATOM_ARRAY_SIZE)
			kring.head = 0;
		*/
	}
	spin_unlock_irqrestore(&kring.dq_lock, flags);
...
void kbase_jd_done_worker_loop(struct work_struct *data)
{
	...
	while(1) {
		katom = kbase_kring_dequeue();
		if (katom == NULL) {
			/* Added timer to prevent queue_work from being ignored
			 * due to synchronization issue.(1 jiffy : 4msec)
			 */
			mod_timer(&kring.remain_timer, get_jiffies_64() + 2);
...
#ifdef CONFIG_MALI_JD_RINGBUFFER
	kbase_kring_enqueue(katom);
	if (kring.is_working == 0) {
		kring.is_working = 1;
		del_timer(&kring.remain_timer);
		INIT_WORK(&katom->work, kbase_jd_done_worker_loop);
		queue_work(job_done_loop_wq, &katom->work);
	}
#else
...
#ifdef CONFIG_MALI_JD_RINGBUFFER
int kbase_kring_init(struct kbase_device *kbdev)
{
	int ret = 0;

	pr_info("Initialization Ringbuffer for job_done.\n");

	kring.head = 0;
	kring.tail = 0;
	kring.is_working = 0;
	/* spin_lock_init(&kring.eq_lock); */
	spin_lock_init(&kring.dq_lock);
	job_done_loop_wq = alloc_workqueue("mali_jd_loop",
			WQ_HIGHPRI | WQ_UNBOUND, 1);
	if (NULL == job_done_loop_wq) {
		ret = -ENOMEM;
	}

	kring.kbdev = kbdev;
	INIT_WORK(&kring_temp_katom.work, kbase_jd_done_worker_loop);
	timer_setup(&kring.remain_timer, kring_timer_func, 0);

	return ret;
}
KBASE_EXPORT_TEST_API(kbase_kring_init);
#endif
