...
/* MALI_SEC_INTEGRATION */
#include "./platform/exynos/gpu_dvfs_handler.h"
...
static int kbase_open(struct inode *inode, struct file *filp)
{
	...
	/* MALI_SEC_INTEGRATION */
#ifdef CONFIG_MALI_DUMMY_JOB_WA
	static bool first_kbase_open = true;
	int err = 0;
#endif
	...
	/* MALI_SEC_INTEGRATION */
#ifdef CONFIG_MALI_DUMMY_JOB_WA
	if (unlikely(first_kbase_open)) {
		err = kbase_dummy_job_wa_load(kbdev);

		if (err) {
			kbase_dummy_job_wa_cleanup(kbdev);
			dev_err(kbdev->dev,
					"Dummy job WA firmware load failed. error = %d\n", err);
		}
		else {
			dev_info(kbdev->dev,
					"Dummy job WA firmware loaded\n");
			first_kbase_open = false;
		}
	}
#endif
...
/* MALI_SEC_INTEGRATION */
static int kbase_api_amigo_flags(struct kbase_context *kctx,
		struct kbase_ioctl_slsi_amigo_flags *flags)
{
	struct exynos_context *platform = NULL;

	if (!kctx || !kctx->kbdev || !kctx->kbdev->platform_context)
		return -ENODEV;

	platform = (struct exynos_context *)kctx->kbdev->platform_context;
#ifdef CONFIG_MALI_TSG
	flags->flags = (uint32_t)platform->amigo_flags;
#endif
	return 0;
}
...
/* MALI_SEC_INTEGRATION */
extern struct gpu_data gpud;

/* MALI_SEC_INTEGRATION */
static int kbase_api_util_info(struct kbase_context *kctx,
		struct kbase_ioctl_slsi_util_info *info)
{
	struct exynos_context *platform = NULL;
	...
	info->flimit = platform->interactive.highspeed_clock;
	info->hcm_mode = platform->heavy_compute_mode;
#endif
	return 0;
}

/* MALI_SEC_INTEGRATION */
static int kbase_api_pmqos(struct kbase_context *kctx, struct kbase_ioctl_slsi_pmqos *pmqos)
{
	struct exynos_context *platform = NULL;
...
#ifdef CONFIG_MALI_DVFS
	if (platform->heavy_compute_mode != pmqos->mode) {
		platform->heavy_compute_mode = pmqos->mode;
...
/* MALI_SEC_INTEGRATION */
static int kbase_api_buffer_alloc_type(struct kbase_context *kctx,
		struct kbase_ioctl_slsi_buffer_alloc_type *buffer_alloc_type)
{
	struct exynos_context *platform = NULL;
	...
	platform = (struct exynos_context *)kctx->kbdev->platform_context;
#ifdef CONFIG_MALI_BUFFER_ALLOC_TYPE
	buffer_alloc_type->type = (uint32_t)platform->buffer_alloc_type;
#endif
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	/* MALI_SEC_INTEGRATION */
	case KBASE_IOCTL_SLSI_BUFFER_ALLOC_TYPE:
		KBASE_HANDLE_IOCTL_OUT(KBASE_IOCTL_SLSI_BUFFER_ALLOC_TYPE,
				kbase_api_buffer_alloc_type,
				struct kbase_ioctl_slsi_buffer_alloc_type,
				kctx);
		break;

	/* MALI_SEC_INTEGRATION */
	case KBASE_IOCTL_SLSI_AMIGO_FLAGS:
		KBASE_HANDLE_IOCTL_OUT(KBASE_IOCTL_SLSI_AMIGO_FLAGS,
				kbase_api_amigo_flags,
				struct kbase_ioctl_slsi_amigo_flags,
				kctx);
		break;

	/* MALI_SEC_INTEGRATION */
	case KBASE_IOCTL_SLSI_UTIL_INFO:
		KBASE_HANDLE_IOCTL_OUT(KBASE_IOCTL_SLSI_UTIL_INFO,
				kbase_api_util_info,
				struct kbase_ioctl_slsi_util_info,
				kctx);
		break;
	...
	/* MALI_SEC_INTEGRATION */
	case KBASE_IOCTL_SLSI_PMQOS:
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_SLSI_PMQOS,
				kbase_api_pmqos,
				struct kbase_ioctl_slsi_pmqos,
				kctx);
		break;
	}
...
{
#if IS_ENABLED(CONFIG_MALI_COHERENCY)
#ifdef CONFIG_OF
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	/* MALI_SEC_INTEGRATION */
#ifdef CONFIG_MALI_JD_RINGBUFFER
	/* Initialization Ringbuffer for job_done */
	kbase_kring_init(kbdev);
#endif
	}
