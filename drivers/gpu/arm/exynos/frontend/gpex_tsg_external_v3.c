...
unsigned long exynos_profiler_get_job_state_cnt(void)
{
	return tsg_info->input_job_nr_acc;
}
EXPORT_SYMBOL(exynos_profiler_get_job_state_cnt);

int exynos_profiler_get_gpu_cur_idx(void)
{
	int i;
	int level = -1;
	int clock = 0;
...
EXPORT_SYMBOL(exynos_profiler_get_gpu_cur_idx);

int exynos_profiler_get_gpu_coeff(void)
{
	int coef = 6144;

	return coef;
}
EXPORT_SYMBOL(exynos_profiler_get_gpu_coeff);

uint32_t exynos_profiler_get_gpu_table_size(void)
{
	return (gpex_clock_get_table_idx(gpex_clock_get_min_clock()) -
		gpex_clock_get_table_idx(gpex_clock_get_max_clock()) + 1);
}
EXPORT_SYMBOL(exynos_profiler_get_gpu_table_size);

static uint32_t freqs[DVFS_TABLE_ROW_MAX];
uint32_t *exynos_profiler_get_freq_table(void)
{
	int i;
	int idx_max_clk, idx_min_clk;
...
uint32_t *exynos_profiler_get_gpu_volt_table(void)
{
	int i;
	int idx_max_clk, idx_min_clk;
...
EXPORT_SYMBOL(exynos_profiler_get_gpu_volt_table);
...
int exynos_profiler_set_queued_threshold_0(uint32_t threshold)
{
	gpex_tsg_set_queued_threshold(0, threshold);
	return gpex_tsg_get_queued_threshold(0);
}
EXPORT_SYMBOL(exynos_profiler_set_queued_threshold_0);

int exynos_profiler_set_queued_threshold_1(uint32_t threshold)
{
	gpex_tsg_set_queued_threshold(1, threshold);

	return gpex_tsg_get_queued_threshold(1);
}
EXPORT_SYMBOL(exynos_profiler_set_queued_threshold_1);

ktime_t *exynos_profiler_get_job_queue_count(void)
{
	int i;
	for (i = 0; i < 2; i++) {
...
EXPORT_SYMBOL(exynos_profiler_get_job_queue_count);

ktime_t exynos_profiler_get_job_queue_last_updated(void)
{
	return gpex_tsg_get_queued_last_updated();
}
EXPORT_SYMBOL(exynos_profiler_get_job_queue_last_updated);

void exynos_profiler_set_gpu_polling_speed(int polling_speed)
{
	gpex_dvfs_set_polling_speed(polling_speed);
}
EXPORT_SYMBOL(exynos_profiler_set_gpu_polling_speed);

int exynos_profiler_get_gpu_polling_speed(void)
{
	return gpex_dvfs_get_polling_speed();
}
EXPORT_SYMBOL(exynos_profiler_get_gpu_polling_speed);
...
void exynos_profiler_set_freq_margin(int margin)
{
...
void exynos_profiler_get_pid_list(u16 *pidlist)
{
...
void exynos_profiler_stc_set_powertable(int id, int cnt, struct freq_table *table)
{
...
static ssize_t show_feedback_governor_impl(char *buf)
{
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			" +- unsigned int exynos_profiler_get_gpu_table_size(void)\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "     +- int exynos_profiler_get_step(void)\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- %u\n",
			exynos_profiler_get_gpu_table_size());
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			" +- unsigned int exynos_profiler_get_gpu_cur_idx(void)\n");

	ret += snprintf(buf + ret, PAGE_SIZE - ret, "     +- int exynos_profiler_get_cur_level(void)\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- clock=%u\n",
			gpex_clock_get_cur_clock());
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- level=%d\n",
			exynos_profiler_get_gpu_cur_idx());
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			" +- unsigned int exynos_profiler_get_gpu_coeff(void)\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"     +- int exynos_profiler_get_coefficient_value(void)\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- %u\n",
			exynos_profiler_get_gpu_coeff());
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			" +- unsigned int *exynos_profiler_get_freq_table(void)\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"     +- uint32_t *exynos_profiler_get_freqs(void)\n");
	freqs = exynos_profiler_get_freq_table();
	for (i = 0; i < exynos_profiler_get_gpu_table_size(); i++) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- %u\n", freqs[i]);
	}
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			" +- unsigned int *exynos_profiler_get_gpu_volt_table(void)\n");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"     +- unsigned int *exynos_profiler_get_volts(void)\n");
	volts = exynos_profiler_get_gpu_volt_table();
	for (i = 0; i < exynos_profiler_get_gpu_table_size(); i++) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- %u\n", volts[i]);
	}
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"     +- ktime_t *exynos_profiler_get_time_in_state(void)\n");
	time_in_state = exynos_profiler_get_time_in_state();
	for (i = 0; i < exynos_profiler_get_gpu_table_size(); i++) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- %lld\n", time_in_state[i]);
	}
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			" +- ktime_t *exynos_profiler_get_job_queue_count(void)\n");
	time_in_state = exynos_profiler_get_job_queue_count();
	for (i = 0; i < 2; i++) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- %lld\n", time_in_state[i]);
	}
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "         +- %d\n",
			exynos_profiler_get_gpu_polling_speed());
