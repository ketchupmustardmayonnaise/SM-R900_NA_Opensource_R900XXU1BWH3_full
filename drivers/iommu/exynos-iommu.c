...
static irqreturn_t exynos_sysmmu_irq(int irq, void *dev_id)
{
	...
	ret = show_fault_information(drvdata, flags, addr, panic_str);
	if (ret == -EAGAIN)
		return IRQ_HANDLED;
	atomic_notifier_call_chain(&drvdata->fault_notifiers, addr, &flags);

	panic("(%s) %s\n", dev_name(drvdata->sysmmu), panic_str);
				...
				struct sysmmu_drvdata *drvdata)
{
	...
	if (of_property_read_bool(sysmmu->of_node, "sysmmu,explicit-disable"))
		drvdata->set_disable = true;
