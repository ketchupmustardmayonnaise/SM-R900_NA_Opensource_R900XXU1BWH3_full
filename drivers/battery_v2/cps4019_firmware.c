...
static int app1_crc_check(struct sb_wrl_fw *fw, int fw_flag)
{
...
program_app1:
	for (; retry_cnt < 10; retry_cnt++) {
		...
		do {
			ret = app_program(fw, &fw->app1_wa, 0);
...
static int run_fw(struct sb_wrl_fw *fw, struct sb_wrl_fw_data *app_bl, struct sb_wrl_fw_data *app, unsigned int fw_flag)
{
	int ret;
	...
	ret = app1_crc_check(fw, fw_flag);
	if (ret != SB_WRL_FW_RESULT_PASS)
		return ret;

	ret = app_program(fw, app, fw_flag);
	if (ret != SB_WRL_FW_RESULT_PASS)
		return ret;
...
static int load_fw_by_bin(struct sb_wrl_fw_data *app_bl, struct sb_wrl_fw_data *app)
{
	...
	ret = copy_fw_data(app_bl, CPS4019_APP2_BL, CPS4019_APP2_BL_SIZE);
	if (ret) {
		fw_log("failed to copy app2 bl - ret(%d)\n", ret);
		return ret;
	}

	ret = copy_fw_data(app, CPS4019_APP2, CPS4019_APP2_SIZE);
	if (ret) {
		fw_log("failed to copy app2 - ret(%d)\n", ret);
		return ret;
	}
...
static void cb_work(struct work_struct *work)
{
	...
	init_fw_data(&fw->app1_wa, SB_WRL_RX_SDCARD_MODE);
	fw_log("run fw - mode = %d\n", fw->mode);
	switch (fw->mode) {
	case SB_WRL_RX_BUILT_IN_MODE:
		/* try to update app2 only */
		if (!load_fw_by_bin(&app_bl, &app) &&
			!copy_fw_data(&fw->app1_wa, CPS4019_APP1, CPS4019_APP1_SIZE)) {
			state = run_fw(fw, &app_bl, &app, (FW_FLAG_APP_TYPE | FW_FLAG_APP_ERASE));
		}
	...
	case SB_WRL_RX_SDCARD_MODE:
		...
		if (!load_fw_by_sdcard(CPS4019_APP1_BL_PATH, &app_bl, CPS4019_APP1_PATH, &app) &&
			!copy_fw_data(&fw->app1_wa, app.data, app.size)) {
			state = run_fw(fw, &app_bl, &app, FW_FLAG_APP_ERASE);
		}
		init_fw_data(&app_bl, SB_WRL_RX_SDCARD_MODE);
		init_fw_data(&app, SB_WRL_RX_SDCARD_MODE);
