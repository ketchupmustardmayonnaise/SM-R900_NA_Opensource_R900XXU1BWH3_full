...
static int sec_bat_get_wireless_download_mode(void)
{
	static int wirelessd = -1;
	char *wirelessd_str = NULL;

	if (wirelessd >= 0)
		return wirelessd;

	wirelessd = 0;
	wirelessd_str = strstr(saved_command_line, "androidboot.wirelessd=");
	if (wirelessd_str != NULL) {
		sscanf(wirelessd_str, "androidboot.wirelessd=%d", &wirelessd);
		pr_info("%s: androidboot.wirelessd = %d\n", __func__, wirelessd);
	}
...
bool sec_bat_check_full(struct sec_battery_info *battery, int full_check_type)
{
	...
	if ((!ret) || ((++battery->full_check_cnt) >= battery->pdata->full_check_count))
		battery->full_check_cnt = 0;
...
static void sec_bat_calculate_safety_time(struct sec_battery_info *battery)
{
	...
	if ((battery->charger_mode != SEC_BAT_CHG_MODE_CHARGING) ||
		sec_bat_get_phm_status(battery) ||
...
static void sec_bat_parse_mode_dt(struct sec_battery_info *battery)
{
	...
	sec_battery_platform_data_t *pdata = battery->pdata;
	int ret = 0, len = 0;
	bool is_recovery_mode = false, is_wirelessd_mode = false;
	const u32 *p;

	is_recovery_mode = (sec_bat_get_boot_mode() == 2);
	is_wirelessd_mode = sec_bat_get_wireless_download_mode();
	battery->store_mode |= (is_recovery_mode | is_wirelessd_mode);
