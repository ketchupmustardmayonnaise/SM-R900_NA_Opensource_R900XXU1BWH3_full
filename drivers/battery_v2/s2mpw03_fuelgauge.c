...
static void s2mpw03_fg_clock_switching_wa(struct s2mpw03_fuelgauge_data *fuelgauge)
{
	pr_info("%s: start\n", __func__);

	/* FG Clock Switching W/A
	 * 1. 0x17[6] = 0
	 * 2. 0x25 = 0x78 : FG 500K Gating disable
	 * 3. Common Reg 0x0D = 0x40 : PM OSC Manual enable on (8M clock enable)
	 * 4. Delay 200us
	 * 5. 0x25 = 0x70 : FG 500K Gating enable
	 * 6. Common Reg 0x0D = 0x00 : PM OSC Manual enable off
	 */
	s2mpw03_fg_update_reg(fuelgauge, S2MPW03_FG_REG_CONFIG2, 0, 0x40);
...
static int s2mpw03_get_vbat(struct s2mpw03_fuelgauge_data *fuelgauge)
{
	...
	if (s2mpw03_check_stuck_wa(fuelgauge, compliment)) {
		...
		s2mpw03_fg_clock_switching_wa(fuelgauge);
		s2mpw03_runtime_dumpdone(fuelgauge);
