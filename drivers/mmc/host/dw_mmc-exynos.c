...
void dw_mci_reg_dump(struct dw_mci *host)
{
	...
	pr_err("DWMCI RAW RAMDUMP FMP-----------------\n");
	for (i = 0x1000; i < 0x1200; i += 0x10) {
		pr_err("%08x %08x %08x %08x\n",
				readl_relaxed(host->regs + i),
				readl_relaxed(host->regs + i + 4),
				readl_relaxed(host->regs + i + 8),
...
#if defined(CONFIG_BCM43013) || defined(CONFIG_BCM43012)
			/* In case of Merlot, dwmmc2 is SDIO for Broadcom Wi-Fi chipset */
			if (id == 2)
		...
		struct device_attribute *attr, char *buf)
{
	...
	err_log = host->err_log;

	for (i = 0; i < 6; i++) {
		if (err_log[i].err_type == -EILSEQ && total_c_cnt < MAX_CNT_U64)
			total_c_cnt += err_log[i].count;
		if ((err_log[i].err_type == -ETIMEDOUT || err_log[i].err_type == -ENOMEDIUM) && total_t_cnt < MAX_CNT_U64)
...
static ssize_t mmc_err_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dw_mci *host = dev_get_drvdata(dev);
	struct mmc_card *card = host->slot->mmc->card;
	...
	if (!card) {
		len = snprintf(buf, PAGE_SIZE, "It's no card error..\n");
		goto out;
	}
	...
	len = snprintf(buf, PAGE_SIZE,
				"type : err first_issue_time:  last_issue_time:      count\n");

	for (i = 0; i < 6; i++) {
		len += snprintf(buf + len, PAGE_SIZE,
				"%5s:%4d %16llu, %16llu, %10d\n",
...
{
	struct dw_mci *host = dev_get_drvdata(dev);
	struct mmc_card_error_log *err_log = host->err_log;
...
static DEVICE_ATTR(err_count, 0444, mmc_err_count_show, NULL);
...
static void dw_mci_exynos_add_sysfs(struct dw_mci *host)
{
	...
	if (of_alias_get_id(host->dev->of_node, "mshc") == 0) {
		if (!mmc_card_dev) {
			...
			if (device_create_file(mmc_card_dev,
					&dev_attr_err_count) < 0)
				pr_err("%s : Failed to create device file(%s)!\n",
					__func__, dev_attr_err_count.attr.name);

			if (device_create_file(mmc_card_dev,
					&dev_attr_ic) < 0)
				pr_err("%s : Failed to create device file.(%s)!\n",
					__func__, dev_attr_ic.attr.name);

			if (device_create_file(mmc_card_dev,
					&dev_attr_lc) < 0)
				pr_err("%s : Failed to create device file.(%s)!\n",
					__func__, dev_attr_lc.attr.name);
#ifdef CONFIG_SEC_FACTORY
			if (device_create_file(mmc_card_dev,
					&dev_attr_hwrst) < 0)
				pr_err("%s : Failed to create device file(%s)!\n",
