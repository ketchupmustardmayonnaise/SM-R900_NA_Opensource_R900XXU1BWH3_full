...
#define STATUS_ERR_MASK     (R1_ERROR | R1_CC_ERROR | R1_CARD_ECC_FAILED | R1_WP_VIOLATION | R1_OUT_OF_RANGE | \
				HALT_UNHALT_ERR | CQ_EN_DIS_ERR | RPMB_SWITCH_ERR | HW_RST)
...
struct mmc_card_error_log {
	...
	u32	hw_rst_cnt;	// HW reset
	unsigned int ic_info; // Init count
	unsigned int lc_info; // LC count
};
...
#define DW_MMC_REQ_BUSY		1
	...
	struct mmc_card_error_log err_log[6];
};
...
void dw_mci_mmc_error_logging(struct dw_mci *host, u32 status, int err, u32 type, bool forced);
