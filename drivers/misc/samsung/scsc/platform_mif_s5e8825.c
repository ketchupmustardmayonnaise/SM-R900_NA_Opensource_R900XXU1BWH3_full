...
#ifdef CONFIG_SCSC_QOS
#if (KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE)
	struct qos_table    table = platform_mif_pm_qos_get_table(platform, SCSC_QOS_MED);
#endif
#endif
...
#ifdef CONFIG_SCSC_QOS
#if (KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE)
			/* Platform Default min INT freq is 50000. With this
			 * freq when wlbt is turned on we see data aborts in Rf
			 * testing and in wifi throughput testing. It was
			 * advised to set the min INT freq to 400000 when wlbt
			 * is turned-on. And remove this req on wlbt off.
			 */
			exynos_pm_qos_add_request(&platform->pm_qos_int_req, PM_QOS_DEVICE_THROUGHPUT, table.freq_int);
			exynos_pm_qos_update_request(&platform->pm_qos_int_req, table.freq_int);
			SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev,
					  "Add pm qos req for INT freq=%d\n", table.freq_int);
#endif
#endif
...
#ifdef CONFIG_SCSC_QOS
#if (KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE)
			/* Remove the Forced INT Freq req.*/
			exynos_pm_qos_remove_request(&platform->pm_qos_int_req);
			SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev,
					  "Remove pm qos req for INT\n");
#endif
#endif
		}
	} else {
